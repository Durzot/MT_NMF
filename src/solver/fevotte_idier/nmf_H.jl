...
Implementation of the Heuristic version of maximization-mimization algorithm under various constraints and for the β divergence.
"""
...
"""
    nmf_H(V, W_init, H_init, params)

Run the Heuristic algorithm using the set of input parameters. For the parameters, see [`FIParams`](@ref)

# References
- C. Fevotte & J. Idier, "Algorithms for nonnegative matrix factorization
with the beta-divergence ", Neural Compuation, 2011.
"""
function nmf_H(V::Matrix{T}, W_init::Matrix{T}, H_init::Matrix{T}, global_params::AbstractNMFParams, local_params::AbstractFIParams) where T <: Real
    W       = W_init[:,:]
    H       = H_init[:,:]
    V_a     = W * H

    #### metrics and stopping_crit
    metrics            = NMFMetrics(V, W_init, H_init, _β_cost, global_params, local_params)
    ...
    while !stopping_crit_met && iteration_is_sane 
        ...
        if global_params.verbose && (metrics.cur_iter % (global_params.max_iter ÷ 10) == 0)
            @printf("iter %d/%d | cost: %.5g\n", metrics.cur_iter, global_params.max_iter, metrics.absolute_cost[metrics.cur_iter])
        end
    end
    ...
    if !iteration_is_sane
        ...
        if metrics.cur_iter < global_params.max_iter
